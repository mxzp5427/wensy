...
class Think extends TagLib{
    ...
    protected $tags   =  array(
		...
		'banner'	=>  array('attr'=>'position,name', 'close'=>1),
        );
    ...
    public function _banner($tag, $content){
		$name     =  'Banner';
		$position = empty($tag['position']) ? 0 : $tag['position'];
		$parseStr = '<?php ';
        $parseStr .= ' $__BANNER__ = M("'.$name.'")->where("position='.$position.' AND status = 1")->order("sort")->select(); ?>';
		$parseStr .= '<volist name="__BANNER__" id="'. $tag['name'] .'">';
		$parseStr .= $content;
		$parseStr .= '</volist>';
        return $parseStr;
    }
