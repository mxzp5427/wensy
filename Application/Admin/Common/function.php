...
/**
 * 获取banner位置名称
 * @param intger $position_id 位置ID
 * @author 墨羲 <moxi@adagl.com>
 */
function get_banner_position( $position_id ) {
	$positions = C('BANNER_POSITION');
	return $positions[$position_id];
}

/**
 * 获取文章推荐位
 * @param intger $position 位置ID
 * @author 墨羲 <moxi@adagl.com>
 */
function get_article_position( $position ) {
	$str = '';
	if(check_document_position($position, 1)) { //列表推荐
		$str .= '<font style="color:#f00; margin-right:10px;">列</font>';
	}
	
	if(check_document_position($position, 2)) { //频道推荐
		$str .= '<font style="color:#0f0; margin-right:10px;">频</font>';
	}
	
	if(check_document_position($position, 4)) { //首页推荐
		$str .= '<font style="color:#00f; margin-right:10px;">首</font>';
	}
	return $str;
}
