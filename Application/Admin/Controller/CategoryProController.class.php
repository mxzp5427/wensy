 ...
 */
class CategoryProController extends AdminController {
    ...
    public function index(){
        $tree = D('CategoryPro')->getTree(0,'id,name,title,sort,pid,allow_publish,status');
    ...
    public function remove(){
        ...
        $child = M('CategoryPro')->where(array('pid'=>$cate_id))->field('id')->select();
        if(!empty($child)){
        ...
        //删除该分类信息
        $res = M('CategoryPro')->delete($cate_id);
        if($res !== false){
            //记录行为
            action_log('update_categorypro', 'categorypro', $cate_id, UID);
            $this->success('删除分类成功！');
        }else{
            $this->error('删除分类失败！');
        }
    ...
    public function operate($type = 'move'){
        ...
        $list = M('CategoryPro')->where($map)->field('id,title')->select();
    ...
    public function move(){
        $to = I('post.to');
        $from = I('post.from');
        $res = M('CategoryPro')->where(array('id'=>$from))->setField('pid', $to);
        if($res !== false){
