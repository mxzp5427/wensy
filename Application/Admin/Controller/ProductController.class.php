...
class ProductController extends AdminController {
    ...
    protected function getMenu(){
        ...
        $cate       =   M('CategoryPro')->where(array('status'=>1))->field('id,title,pid,allow_publish')->order('pid,sort')->select();
    ...
    public function index($cate_id = null){
        ...
        if(!empty($cate_id)){   //没有权限则不查询数据
            //获取分类绑定的模型
            $models         =   get_category($cate_id, 'model', 'pro');
            $allow_reply    =   get_category($cate_id, 'reply', 'pro');//分类文档允许回复
    ...
    protected function indexOfReply($cate_id) {
        ...
        //检查该分类是否允许发布内容
        $allow_publish  =   get_category($cate_id, 'allow_publish', 'pro');
